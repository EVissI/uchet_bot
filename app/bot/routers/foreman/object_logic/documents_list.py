...
async def handle_workers_list(callback: CallbackQuery, callback_data: ForemanObjectCallback, user_info: User) -> None:
    ...
    async with async_session_maker() as session:
        documents:list[ObjectDocument] = await ObjectDocumentDAO.find_object_documents(session, object_id)
        selected_object:list[Object] = await ObjectDAO.find_one_or_none(session, object_id)
