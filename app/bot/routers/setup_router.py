...
from app.bot.routers.general_routers.cheks_out_object import checks_out_object_check_router
...
async def finish_photos(message: Message, state: FSMContext):
    ...
    await message.answer(
        get_text('i_got_acquainted', lang=lang),reply_markup=i_got_acquainted_kbds(lang=lang)
    )
