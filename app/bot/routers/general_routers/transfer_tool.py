...
async def transfer_tool(message: Message, user_info: User) -> None:
    """
    Передача инструмента: ввод ID инструмента, Telegram ID или username получателя,
    загрузка фото и описание — сообщение пересылается в Telegram-группу.

    Формат команды:
      /transfer_tool <инструмент_ID> <получатель> <описание>

    Фото можно прикрепить к сообщению.
    """
...
@transfer_router.callback_query(AcceptToolCallback.filter(),UserInfo())
async def accept_tool(
    ...
    async with async_session_maker() as session:
        ...
        if tool:
            ...
            await ToolDAO.update(session, filters=ToolFilterModel(id=tool.id),values=ToolFilterModel.model_validate(tool.to_dict()))
    await callback_query.message.edit_text(get_text("tool_received_confirmation", user_info.language))
