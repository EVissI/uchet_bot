...
async def create_material_reminder_kb(message: Message, state:FSMContext,user_info: User):
    await state.set_state(AdminPanelStates.material_reminder_setup)
    await message.answer(
        text=message.text,
        reply_markup=AdminMaterialReminderKeyboard.get_material_reminder_setup_kb(user_info.language)
    )

@reminder_setup_router.message(F.text.in_(get_all_texts("back_btn")),StateFilter(AdminPanelStates.material_reminder_setup),UserInfo())
